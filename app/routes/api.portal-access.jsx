...
export const action = async ({ request }) => {
  ...
  try {
    ...
    if (intent === "save-customer-policy") {
      ...
      if (artistMode === "SOLO") {
        const artist = soloArtistId ? await findShopArtist(session.shop, soloArtistId) : null;
    ...
    const release = await findShopRelease(session.shop, releaseId, { include:{ artists:true } });
    ...
    await db.$transaction(async (tx) => {
      await tx.release.updateMany({ where:{ id:release.id, shop:session.shop }, data:{ ownerCustomerId } });
    ...
    return apiErrorResponse(request, error, { context: "portal access mutation", fallback: "Could not update portal access." });
  }
};
